...
def train(args):
  ...
  train_dataset = load_data(f)
  # dev_dataset = load_data(f2)
  print(train_dataset.shape)
  # _,dev_dataset = train_test_split(train_dataset,test_size=0.2,shuffle=True)
  print(train_dataset.shape)
  ...
  for idx, (train_idx,val_idx) in enumerate(cv.split(train_dataset, train_dataset['label'].values)):
    t_dataset = train_dataset.iloc[train_idx]
    val_dataset = train_dataset.iloc[val_idx]
    print(t_dataset)
    print(val_dataset)
    tokenized_train = tokenized_dataset(t_dataset, tokenizer)
